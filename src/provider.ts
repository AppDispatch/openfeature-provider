...
import type { DispatchProviderOptions, FlagDefinition, FlagPayload } from './types'
import { evaluateFlag } from './evaluator'

export class DispatchProvider implements CommonProvider<ClientProviderStatus> {
  readonly metadata: ProviderMetadata = { name: 'dispatch-ota' }
  ...
  private async fetchFlags(): Promise<void> {
    try {
      ...
      if (!res.ok) {
        console.warn(`[dispatch-ota] Failed to fetch flags: ${res.status}`)
        return
      }
      ...
      console.warn('[dispatch-ota] Failed to fetch flags:', err)
    }
  }
